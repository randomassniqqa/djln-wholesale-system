...
--
-- Host: 127.0.0.1
-- Generation Time: May 24, 2026 at 10:26 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `tx_CompleteWholesaleOrder` (IN `p_customer_id` BIGINT, IN `p_product_id` BIGINT, IN `p_quantity` INT)   BEGIN 
                DECLARE v_order_id BIGINT; DECLARE v_unit_price DECIMAL(10,2); DECLARE v_stock_qty INT; DECLARE v_order_number VARCHAR(255); 
                DECLARE EXIT HANDLER FOR SQLEXCEPTION BEGIN ROLLBACK; RESIGNAL; END; 
                START TRANSACTION; 
                SELECT stock_quantity, wholesale_price INTO v_stock_qty, v_unit_price FROM products WHERE id = p_product_id FOR UPDATE; 
                IF v_stock_qty < p_quantity THEN ROLLBACK; SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Insufficient stock for this product.'; END IF; 
                SET v_order_number = CONCAT('WHS-', DATE_FORMAT(NOW(), '%Y%m%d%H%i%s')); 
                INSERT INTO orders (order_number, customer_id, order_type, order_status, payment_status, ordered_at, created_at, updated_at) 
                VALUES (v_order_number, p_customer_id, 'wholesale', 'pending', 'unpaid', NOW(), NOW(), NOW()); 
                SET v_order_id = LAST_INSERT_ID(); 
                INSERT INTO order_items (order_id, product_id, quantity, unit_price, line_total, created_at, updated_at) 
                VALUES (v_order_id, p_product_id, p_quantity, v_unit_price, (p_quantity * v_unit_price), NOW(), NOW()); 
                UPDATE products SET stock_quantity = stock_quantity - p_quantity, stock_status = CASE WHEN (stock_quantity - p_quantity) <= 0 THEN 'out_of_stock' WHEN (stock_quantity - p_quantity) <= reorder_level THEN 'low_stock' ELSE 'in_stock' END WHERE id = p_product_id; 
                COMMIT; 
            END$$
...
(8, 18, 'fulfillment_status_change', 'Order', 5, '{\"before\": {\"status\": \"pending\"}, \"after\": {\"status\": \"shipped\"}}', NULL, NULL, '2026-05-24 18:42:22', '2026-05-24 18:42:22'),
(9, 18, 'fulfillment_status_change', 'Order', 5, '{\"before\": {\"status\": \"shipped\"}, \"after\": {\"status\": \"pending\"}}', NULL, NULL, '2026-05-24 18:43:08', '2026-05-24 18:43:08'),
(10, 18, 'fulfillment_status_change', 'Order', 5, '{\"before\": {\"status\": \"pending\"}, \"after\": {\"status\": \"delivered\"}}', NULL, NULL, '2026-05-24 18:43:12', '2026-05-24 18:43:12'),
(11, 18, 'stock_deduction', 'Product', 5, '{\"before\": {\"qty\": \"57\"}, \"after\": {\"qty\": 47, \"order_id\": \"6\"}}', NULL, NULL, '2026-05-24 18:44:08', '2026-05-24 18:44:08');
...
(27, '2026_05_24_184843_rename_columns_for_normalization', 5),
(28, '2026_05_25_000002_rename_erd_columns_for_normalization', 6);
...
('0c3d806d-3040-4050-93b2-51a8abcba80d', 'App\\Notifications\\OrderStatusNotification', 'App\\Models\\User', 18, '{\"message\":\"Your order DJLN-2026-00005 is on its way! \\ud83d\\ude9a Expect delivery soon.\",\"type\":\"order\",\"category\":\"order\",\"order_id\":5,\"order_number\":\"DJLN-2026-00005\",\"status\":\"shipped\",\"status_label\":\"Out for Delivery\",\"status_emoji\":\"\\ud83d\\ude9a\",\"total_amount\":3725,\"previous_status\":\"pending\",\"created_at\":\"2026-05-24T18:42:24.216777Z\"}', '2026-05-24 18:43:36', '2026-05-24 10:42:24', '2026-05-24 10:43:36'),
('13b69c7a-45bd-4155-92a4-973423f34002', 'App\\Notifications\\OrderStatusNotification', 'App\\Models\\User', 18, '{\"message\":\"Your order DJLN-2026-00005 has been delivered. Thank you for shopping with DJLN! \\u2705\",\"type\":\"order\",\"category\":\"order\",\"order_id\":5,\"order_number\":\"DJLN-2026-00005\",\"status\":\"delivered\",\"status_label\":\"Delivered\",\"status_emoji\":\"\\u2705\",\"total_amount\":3725,\"previous_status\":\"pending\",\"created_at\":\"2026-05-24T18:43:13.057440Z\"}', '2026-05-24 18:43:33', '2026-05-24 10:43:13', '2026-05-24 10:43:33'),
('1c00bf0a-54fa-11f1-ad66-2cf05dd037cd', 'App\\Notifications\\NewOrderAlert', 'App\\Models\\User', 1, '{\"order_id\": 5, \"order_number\": \"DJLN-2026-00005\", \"customer_id\": 18, \"customer_name\": \"Balloon Bash Co.\", \"total_amount\": 0.00, \"order_type\": \"retail\", \"message\": \"New Order #DJLN-2026-00005 received from Balloon Bash Co. for u20b10.00\", \"icon\": \"ud83duded2\", \"created_at\": \"2026-05-21T17:47:41Z\"}', '2026-05-24 18:03:29', '2026-05-21 09:47:41', '2026-05-24 10:03:29'),
...
('7f52babb-ebe8-4537-bafd-d194b22f2f8c', 'App\\Notifications\\OrderStatusNotification', 'App\\Models\\User', 18, '{\"message\":\"Your order DJLN-2026-00003 has been placed successfully! \\ud83c\\udf89 We\'ll start processing it shortly.\",\"type\":\"order\",\"category\":\"order\",\"order_id\":6,\"order_number\":\"DJLN-2026-00003\",\"status\":\"placed\",\"status_label\":\"Order Placed\",\"status_emoji\":\"\\ud83c\\udf89\",\"total_amount\":1500,\"previous_status\":null,\"created_at\":\"2026-05-24T18:44:08.849723Z\"}', NULL, '2026-05-24 10:44:08', '2026-05-24 10:44:08'),
('8c525bf6-57a0-11f1-8213-2cf05dd037cd', 'App\\Notifications\\NewOrderAlert', 'App\\Models\\User', 1, '{\"order_id\": 6, \"order_number\": \"DJLN-2026-00003\", \"customer_id\": 18, \"customer_name\": \"Balloon Bash Co.\", \"total_amount\": 0.00, \"order_type\": \"wholesale\", \"message\": \"New Order #DJLN-2026-00003 received from Balloon Bash Co. for u20b10.00\", \"icon\": \"ud83duded2\", \"created_at\": \"2026-05-25T02:44:08Z\"}', NULL, '2026-05-24 18:44:08', '2026-05-24 18:44:08'),
('b7502f26-8bc9-4b9d-8e37-d8023e975169', 'App\\Notifications\\OrderStatusNotification', 'App\\Models\\User', 18, '{\"message\":\"Your order DJLN-2026-00005 is pending review.\",\"type\":\"order\",\"category\":\"order\",\"order_id\":5,\"order_number\":\"DJLN-2026-00005\",\"status\":\"pending\",\"status_label\":\"Awaiting Review\",\"status_emoji\":\"\\ud83d\\udd50\",\"total_amount\":3725,\"previous_status\":\"shipped\",\"created_at\":\"2026-05-24T18:43:08.215211Z\"}', '2026-05-24 18:43:36', '2026-05-24 10:43:08', '2026-05-24 10:43:36');
...
INSERT INTO `orders` (`id`, `order_number`, `customer_id`, `staff_id`, `order_type`, `order_status`, `payment_status`, `subtotal`, `discount_amount`, `total_amount`, `notes`, `ordered_at`, `created_at`, `updated_at`) VALUES
...
(5, 'DJLN-2026-00005', 18, 18, 'retail', 'delivered', 'paid', 3725.00, 0.00, 3725.00, NULL, '2026-05-21 01:47:41', '2026-05-21 01:47:41', '2026-05-24 10:43:12'),
(6, 'DJLN-2026-00003', 18, 18, 'wholesale', 'pending', 'unpaid', 1500.00, 0.00, 1500.00, NULL, '2026-05-24 10:44:08', '2026-05-24 10:44:08', '2026-05-24 10:44:08');
...
CREATE TRIGGER `tr_new_order_notification` AFTER INSERT ON `orders` FOR EACH ROW BEGIN
                ...
                SELECT name INTO v_customer_name FROM users WHERE id = NEW.customer_id LIMIT 1;
                ...
                SELECT UUID(), 'App\Notifications\NewOrderAlert', 'App\Models\User', u.id,
                    JSON_OBJECT(
                        ...
                        'message',       CONCAT('New Order #', NEW.order_number, ' received from ', v_customer_name, ' for ₱', FORMAT(NEW.total_amount, 2)),
                        'icon',          '?',
...
(13, 6, 5, 10, 150.00, 1500.00, '2026-05-24 10:44:08', '2026-05-24 10:44:08');
...
INSERT INTO `products` (`id`, `category_id`, `name`, `sku`, `description`, `brand_name`, `weight_volume`, `unit`, `wholesale_price`, `retail_price`, `stock_quantity`, `reorder_level`, `moq`, `stock_status`, `expiry_date`, `image_url`, `is_active`, `created_by`, `created_at`, `updated_at`) VALUES
...
(5, 2, 'Classic Round Lollipops — 100 ct', 'LLP-ROUND-100', 'Assorted fruit-flavored round lollipops, 100-count display box.', NULL, NULL, 'box', 150.00, 200.00, 47, 10, 1, 'in_stock', NULL, NULL, 1, 1, '2026-05-17 06:33:52', '2026-05-24 10:44:08'),
...
('TD65RSRNCJVpFoSgQcQ8fL9QJbUI8u9nEH05ZPYi', 18, '127.0.0.1', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/148.0.0.0 Safari/537.36', '', 1779654327);
...
CREATE TABLE `users` (
  ...
  `phone` varchar(20) DEFAULT NULL COMMENT 'PH mobile or landline — e.g. 09171234567',
...
INSERT INTO `users` (`id`, `name`, `email`, `phone`, `email_verified_at`, `password`, `role`, `theme_preference`, `remember_token`, `created_at`, `updated_at`) VALUES
(1, 'Admin', 'admin', NULL, '2026-05-17 06:33:48', '$2y$12$TdE36n4h8uWgrR6n7yHPUuo4ivuyvWgWLCPw/Z/VENV8E/yX2Q7yi', 'admin', 'dark', '1x76IrxwfnLOIYWyWsSBz8g55JLWCF0maZ06JW8gNk127ENC8vlyPTFpMr2f', '2026-05-17 06:33:48', '2026-05-24 12:09:52'),
(2, 'Sarah', 'sarah', NULL, '2026-05-17 06:33:49', '$2y$12$nUj57RnBX1Qji427A.zmFuzTkljQ1YmlIQBhng/dBymj5RWftVsvW', 'project_manager', 'light', '5rD90N2MBMVObrJpNHrPNlUHDaDlIh16PXRAm92TGCqXJ6gG5MAKCTFTzdk9', '2026-05-17 06:33:49', '2026-05-17 06:33:49'),
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_ordersummary`  AS SELECT `o`.`id` AS `order_id`, `o`.`order_number` AS `order_number`, `u`.`name` AS `customer_name`, `o`.`order_type` AS `order_type`, `o`.`order_status` AS `order_status`, `o`.`payment_status` AS `payment_status`, `o`.`total_amount` AS `total_amount`, `o`.`ordered_at` AS `ordered_at` FROM (`orders` `o` left join `users` `u` on(`o`.`customer_id` = `u`.`id`)) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_productstockstatus`  AS SELECT `p`.`id` AS `product_id`, `p`.`sku` AS `sku`, `p`.`name` AS `product_name`, `c`.`name` AS `category_name`, `p`.`stock_quantity` AS `stock_quantity`, `p`.`reorder_level` AS `reorder_level`, CASE WHEN `p`.`stock_quantity` = 0 THEN 'Out of Stock' WHEN `p`.`stock_quantity` <= `p`.`reorder_level` THEN 'Low Stock' ELSE 'Healthy' END AS `stock_health` FROM (`products` `p` left join `categories` `c` on(`p`.`category_id` = `c`.`id`)) ;
...
ALTER TABLE `orders`
  ...
  ADD KEY `orders_processed_by_foreign` (`staff_id`),
  ADD KEY `orders_order_number_index` (`order_number`),
  ADD KEY `orders_customer_id_index` (`customer_id`),
  ADD KEY `orders_fulfillment_status_index` (`order_status`),
  ADD KEY `orders_order_type_index` (`order_type`),
  ADD KEY `orders_payment_status_index` (`payment_status`),
  ADD KEY `orders_customer_id_fulfillment_status_index` (`customer_id`,`order_status`);
...
ALTER TABLE `products`
  ...
  ADD KEY `products_stock_qty_reorder_level_index` (`stock_quantity`,`reorder_level`);
...
ALTER TABLE `orders`
  ...
  ADD CONSTRAINT `orders_processed_by_foreign` FOREIGN KEY (`staff_id`) REFERENCES `users` (`id`) ON DELETE SET NULL;
